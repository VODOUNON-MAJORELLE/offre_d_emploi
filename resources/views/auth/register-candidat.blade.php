...
<head>
  ...
  <title>Talentlink — Inscription candidat</title>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .row-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 1rem; }
    ...
    input:focus { border-color: #5040e8; box-shadow: 0 0 0 3px rgba(80,64,232,0.12); background: #fff; }
    ...
    /* Upload zone */
    .upload-zone { border: 1.5px dashed #c8c8e0; border-radius: 14px; padding: 2rem 1rem; display: flex; flex-direction: column; align-items: center; gap: 8px; cursor: pointer; transition: border-color 0.15s, background 0.15s; margin-bottom: 1.2rem; }
    ...
    @media (max-width: 700px) { .panel-left { display: none; } .panel-right { padding: 2rem 1.5rem; } .row-2 { grid-template-columns: 1fr; } }
  </style>
</head>
<body>
<div class="wrapper">
  <div class="panel-left">
    ...
    <div class="hero">
      <h1>Créez votre profil candidat.</h1>
      <p>Rejoignez 320 000+ candidats qui ont trouvé leur emploi idéal.</p>
      <ul class="features">
  ...
  <div class="panel-right">
        ...
        <form id="step1-form">
          <div class="row-2">
            ...
            <div class="field">
              ...
              <input type="text" id="nom" name="nom" placeholder="Martin" value="{{ old('nom') }}" />
              @error('nom')
                <div style="color: #e24b4a; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="field">
            ...
            <input type="email" id="email" name="email" placeholder="marie@email.com" value="{{ old('email') }}" />
            @error('email')
              <div style="color: #e24b4a; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
            @enderror
          </div>
          <div class="field">
            ...
            <input type="tel" id="telephone" name="telephone" placeholder="06 12 34 56 78" value="{{ old('telephone') }}" />
            @error('telephone')
              <div style="color: #e24b4a; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
            @enderror
          </div>
          <div class="field">
            ...
            <input type="text" id="ville" name="ville" placeholder="Paris" value="{{ old('ville') }}" />
            @error('ville')
              <div style="color: #e24b4a; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
            @enderror
          </div>
    ...
    <div id="step2" style="display:none">
      <div class="card">
        <form method="POST" action="{{ route('register.candidat.submit') }}" enctype="multipart/form-data">
          @csrf
          <input type="hidden" id="step1-prenom" name="prenom" value="{{ old('prenom') }}">
          <input type="hidden" id="step1-nom" name="nom" value="{{ old('nom') }}">
          <input type="hidden" id="step1-email" name="email" value="{{ old('email') }}">
          <input type="hidden" id="step1-telephone" name="telephone" value="{{ old('telephone') }}">
          <input type="hidden" id="step1-ville" name="ville" value="{{ old('ville') }}">
          
          <div class="upload-zone" onclick="document.getElementById('photo-input').click()">
            <div class="up-icon"><i class="ti ti-upload"></i></div>
            <p id="photo-label">Photo de profil</p>
            <span>JPG, PNG — max 5 MB</span>
            <input type="file" id="photo-input" name="photo_profil" accept="image/*" onchange="updatePhotoLabel(this)" />
          </div>
          <div id="photo-error" style="color: #e24b4a; font-size: 12px; margin-bottom: 1.2rem; display: none; font-weight: 500;"></div>
          @error('photo_profil')
    ...
    <div class="bottom-links">
      Déjà un compte ? <a href="{{ route('login') }}">Se connecter</a> · <a href="{{ route('register.entreprise') }}">Inscription entreprise</a>
    </div>
  </div>
</div>
...
<script>
  function goStep2() {
    ...
    if (!prenom || !email) {
      if (!prenom) document.getElementById('prenom').classList.add('error');
      if (!email) document.getElementById('email').classList.add('error');
      return;
    }
    
    // Transfer values to step 2 hidden inputs
    document.getElementById('step1-prenom').value = document.getElementById('prenom').value;
    document.getElementById('step1-nom').value = document.getElementById('nom').value;
    document.getElementById('step1-email').value = document.getElementById('email').value;
    document.getElementById('step1-telephone').value = document.getElementById('telephone').value;
    document.getElementById('step1-ville').value = document.getElementById('ville').value;
  ...
  function updatePhotoLabel(input) {
    ...
    if (input.files.length > 0) {
      ...
      if (file.size > maxSize) {
        ...
        document.getElementById('photo-label').textContent = "Photo de profil";
        return;
      }
  ...
  ['prenom','nom','email','telephone','ville','password','password_confirmation'].forEach(id => {
  ...
  @if($errors->has('password') || $errors->has('password_confirmation') || $errors->has('cgu') || $errors->has('photo_profil'))
