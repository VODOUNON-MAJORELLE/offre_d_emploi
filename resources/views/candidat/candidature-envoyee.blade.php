...
<nav>
  ...
  <div class="nav-right">
    @php
      $initials = strtoupper(substr($candidat->nom ?? '', 0, 1) . substr($candidat->prenom ?? '', 0, 1));
    @endphp
...
<div class="center">
  <div class="box">
    ...
    <div class="conf-sub">
      Votre candidature pour <strong>{{ $titre_offre ?? 'le poste' }}</strong> chez <strong>{{ $nom_entreprise ?? 'l\'entreprise' }}</strong> a bien été envoyée.
    </div>
    ...
    <div class="score-card">
      ...
      <div class="score-row">
        ...
        <span class="score-value">{{ $score_compatibilite ?? 0 }}%</span>
      </div>
      
      @if($score_questionnaire !== null)
      <div class="score-row">
        <span class="score-label">Questionnaire</span>
        <span class="score-value">{{ $score_questionnaire }}%</span>
      </div>
      @endif
      ...
      <div class="score-final">
        ...
        <div class="score-bar-wrap">
          <div class="score-bar-fill" style="width: {{ $score_final }}%"></div>
        </div>
        <span class="score-final-val">{{ $score_final }}%</span>
      </div>
    </div>
    @endif
    
    <div class="conf-hint">Vous pouvez suivre l'avancement de votre candidature depuis votre espace.</div>
    <button class="btn-primary" onclick="window.location='{{ route('candidat.feed') }}'">Retour au feed</button>
    @if(isset($id_offre) && $id_offre)
    <button class="btn-outline" onclick="window.location='{{ route('candidat.offres.postuler', ['id_offre' => $id_offre, 'edit' => 1]) }}'">Modifier la candidature</button>
    @else
    <button class="btn-outline" onclick="window.location='{{ route('candidat.dashboard') }}'">Voir mes candidatures</button>
    @endif
  </div>
</div>
